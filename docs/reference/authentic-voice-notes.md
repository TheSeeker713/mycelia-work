# Writing in a genuine first-person voice — reference guide

This document exists so any AI model working on Mycelia Time (or another
project) can write first-person content — devlogs, the AI-generated work
journal, anything presented as Jeremy Robards' own words — without it
reading as AI-generated. It's meant to be handed to a model directly as
context before it writes anything in this voice.
...
colleague, a real friend), not a broad audience — "if you wouldn't say
it out loud to a friend, don't type it." Write every entry addressed, in
spirit, to one person the author actually knows, not performed for an
...
— rather than a generic abstraction ("I worked hard on the data layer"
vs. naming the actual bug that broke and what the terminal actually
said). Genuine writing also tolerates contradiction and unresolved
feeling (satisfied *and* annoyed at the same time) instead of smoothing
...
purpose; it means not sanding every rough edge off in the name of
tidiness.
...
- Address the entry, in spirit, to one real person the author knows,
  not an audience.
- Anchor each entry in at least one specific, concrete detail from that
  actual session — a real filename, a real error, a real number —
  instead of summarizing in the abstract.
